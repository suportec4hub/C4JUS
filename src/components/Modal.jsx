...
export default function Modal({ title, onClose, children, width = 500 }) {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 20px",borderBottom:`1px solid ${L.lineSoft}`,position:"sticky",top:0,background:L.white,zIndex:1}}>
          <div style={{fontSize:14,fontWeight:700,color:L.t1,fontFamily:"'Outfit',sans-serif"}}>{title}</div>
          <button onClick={onClose}
            style={{background:"none",border:"none",cursor:"pointer",color:L.t4,fontSize:18,lineHeight:1,padding:"2px 6px",borderRadius:6,transition:"all .1s"}}
...
export function Input({ value, onChange, placeholder, type="text", ...rest }) {
  ...
  return (
    <input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} type={type}
      ...
      style={{width:"100%",background:L.surface,border:`1.5px solid ${focus?"#1a3a6b":L.line}`,borderRadius:9,padding:"9px 12px",color:L.t1,fontSize:12.5,fontFamily:"'Instrument Sans',sans-serif",outline:"none",transition:"border-color .12s",...(rest.style||{})}}
      {...rest}
    />
  );
}
...
export function Textarea({ value, onChange, placeholder, rows=3 }) {
  ...
  return (
    <textarea value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} rows={rows}
      onFocus={()=>setFocus(true)} onBlur={()=>setFocus(false)}
      style={{width:"100%",background:L.surface,border:`1.5px solid ${focus?"#1a3a6b":L.line}`,borderRadius:9,padding:"9px 12px",color:L.t1,fontSize:12.5,fontFamily:"'Instrument Sans',sans-serif",outline:"none",transition:"border-color .12s",resize:"vertical"}}
    />
  );
}
...
export function ModalFooter({ onClose, onSave, loading, label="Salvar", danger }) {
  const L2 = danger ? "#dc2626" : "#1a3a6b";
  return (
    <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:20,paddingTop:16,borderTop:`1px solid ${L.lineSoft}`}}>
      <button onClick={onClose}
        ...
        onMouseEnter={e=>e.currentTarget.style.borderColor=L2}
        onMouseLeave={e=>e.currentTarget.style.borderColor=L.line}
      >Cancelar</button>
      <button onClick={onSave} disabled={loading}
        style={{padding:"9px 20px",borderRadius:9,fontSize:12.5,fontWeight:600,cursor:loading?"wait":"pointer",fontFamily:"inherit",background:L2,color:"white",border:"none",opacity:loading?.7:1,transition:"all .12s"}}
      >
